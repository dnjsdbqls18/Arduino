...
void vision_line_control(int base_speed, double l_c)
{
  ...
  motor_L_control(base_speed + pwm_control_value);  // 부호는 반대로
...
void loop()
{
  ...
  error[0]     = waypoint[wp_go_id].pos_x - my_odom.x;        // x 방향 error
  error[1]     = waypoint[wp_go_id].pos_y - my_odom.y;        // y 방향 error
  error[2]     = waypoint[wp_go_id].angle - my_odom.theta;    // 각도 error
  ...
  if (control_type = VLC)
  {
    VLC_move();
    // 다 이동했나 확인
    if ((fabs(waypoint[wp_go_id].angle) == 0) || (fabs(waypoint[wp_go_id].angle) == 180))
    {
      if (fabs (error[0]) < Waypoint_X_Tor)
      {
        //Serial.println("Arrival_waypoint ");
        count = -1;
        wp_go_id++;
      }
    }
    if ((fabs(waypoint[wp_go_id].angle) == 90) || (fabs(waypoint[wp_go_id].angle) == -90))
    {
      if (fabs (error[1]) < Waypoint_Y_Tor)
      {
        //Serial.println("Arrival_waypoint ");
        count = -1;
        wp_go_id++;
  ...
  {
    IHC_move(waypoint[wp_go_id].angle);

    if (fabs (error[2]) < Waypoint_Angle_Tor)
    {
      //Serial.println("Arrival_waypoint ");
      count = -1;
      wp_go_id++;
