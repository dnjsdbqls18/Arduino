...
#define THRESHOLD_ANGLE1 15 //15
#define THRESHOLD_ANGLE2 7  //7
...
/*
  ...
  {
  bool flag = true; // true(참) 또는 false(거짓) 값을 나타내는 bool 타입 변수

  while (flag)
  {
    read_imu_sensor(); // IMU 센서 읽기

    if (heading_angle_error > THRESHOLD_ANGLE1)
    {
      // IMU 각도 오차가 THRESHOLD_ANGLE1보다 크면 반시계방향으로 회전
      motor_control_L(-255);
      motor_control_R(255);
    }
    else if (heading_angle_error >= THRESHOLD_ANGLE2)
    {
      // IMU 각도 오차가 THRESHOLD_ANGLE2 이상 THRESHOLD_ANGLE1 이하이면 계속 반시계방향으로 회전
      motor_control_L(-255);
      motor_control_R(255);
    }
    else if (heading_angle_error > -THRESHOLD_ANGLE2)
    {
      // IMU 각도 오차가 -THRESHOLD_ANGLE2 초과면 정지
      motor_control_L(0);
      motor_control_R(0);
    ...
    {
      // IMU 각도 오차가 -THRESHOLD_ANGLE1 이상 -THRESHOLD_ANGLE2 이하이면 시계방향으로 회전
      motor_control_L(255);
      motor_control_R(-255);
    }
    else
    {
      // 나머지 경우에는 시계방향으로 계속 회전
      motor_control_L(255);
      motor_control_R(-255);
    }
  }
